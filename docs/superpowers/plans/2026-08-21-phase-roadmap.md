...
## Phase 4 — Rx Safety Checker ✅ built
...
**Built:** migration 0006 (drug_reference seeded with 43 Indian-market drugs incl. max daily doses, prescriptions + items, patients.allergies); rules engine with class-level allergy matching (penicillin/sulfa/nsaid), 17 interaction pairs checked within the Rx AND against existing medications, max-dose math; endpoints: drug autocomplete, allergies PUT, check-rx (inline), save (allergy blocks → 400 always; warnings → 409 until acknowledged_warnings), history list; saved Rx auto-creates prescription timeline_event carrying followup_date for Phase 3 reminders. Frontend Rx tab: allergy chip editor, drug search with reference info, live debounced safety report, acknowledge-to-save gate, prescription history. 36 backend tests green; live E2E verified block/409/ack/save/cross-Rx paths.
**Plan:** implemented directly against spec §rx-safety requirements + trust rules.
